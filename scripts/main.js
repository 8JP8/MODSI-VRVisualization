...
// --- CHANGE: Added a comment explaining how to adjust pie chart height ---
// To adjust the height of all pie charts, modify the 'pieOffsetY' value below.
// A larger value will move the pie charts higher off the ground.
...
// --- CHANGE: Modified function to support rotation by removing position parameters. ---
// The buttons container is now positioned relative to a parent group.
function createChartButtons(originalIndex, state) {
const chartConfig = chartsData[originalIndex];
...
// Position is now relative to the parent group, placing it to the left and above the chart.
// Rotation is inherited from the parent, so buttons rotate with the chart.
buttonsContainer.setAttribute('position', '-4 3 0');
...
// --- CHANGE: Heavily modified to use a parent group for chart + buttons. ---
// This enables rotation for buttons and fixes pie chart height from JSON.
...
// Remove the entire old group (chart + buttons) by its group index.
const existingChartGroup = root.querySelector(`a-entity[data-chart-group-index="${originalIndex}"]`);
if(existingChartGroup) existingChartGroup.parentNode.removeChild(existingChartGroup);
...
setTimeout(() => {
    ...
    if (isCustomPositionValid(chart.position)) {
        ...
        // Add Y offset for custom-positioned pie charts to lift them from the ground.
        const yOffset = chart.chartType === 'babia-pie' ? POSITION_CONFIG.pieOffsetY : 0;
        positionStr = `${pos.x * POSITION_SCALING_FACTOR} ${(pos.y * POSITION_SCALING_FACTOR) + yOffset} ${pos.z * POSITION_SCALING_FACTOR}`;
    ...
    // Create a parent group to hold both the chart and its buttons.
    // This group handles the overall position and rotation.
    const chartGroup = document.createElement('a-entity');
    chartGroup.setAttribute('data-chart-group-index', originalIndex);
    chartGroup.setAttribute('position', positionStr);
    chartGroup.setAttribute('rotation', rotationStr);

    // Create chart with a relative position/rotation and add it to the group.
    const newChartEl = createChart(chartConfig, originalIndex, '0 0 0', '0 0 0', scaleStr);
    if (newChartEl) chartGroup.appendChild(newChartEl);
    
    // Create buttons with a relative position and add them to the group.
    const newButtonsEl = createChartButtons(originalIndex, state);
    if (newButtonsEl) chartGroup.appendChild(newButtonsEl);
...
// --- CHANGE: Modified to use a parent group for chart + buttons. ---
// This enables rotation for buttons and fixes pie chart height from JSON.
...
chartsData.forEach((chartConfig, originalIndex) => {
    ...
    if (chartHasAnyData) {
        ...
        // Create chart with a relative position/rotation and add it to the group.
        const el = createChart(chartConfig, originalIndex, '0 0 0', '0 0 0', scaleStr);
        if (el) chartGroup.appendChild(el);
        
        // Create buttons with a relative position and add them to the group.
        const buttonsEl = createChartButtons(originalIndex, state);
        if (buttonsEl) chartGroup.appendChild(buttonsEl);
